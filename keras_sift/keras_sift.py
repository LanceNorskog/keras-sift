...
def getSIFTModel(inputs=None, feed=None, patch_size = 65, num_ang_bins = 4, num_spatial_bins = 4):
    if inputs is None:
        inputs = tf.keras.layers.Input(shape=(patch_size, patch_size, 1))
        feed = inputs
    # assert shape is n, n, 1
    kerassift = get_sift_model(feed, img_rows=patch_size, num_ang_bins=num_ang_bins, num_spatial_bins=num_spatial_bins)
